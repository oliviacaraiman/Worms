...
public class Joueur {
	...
	public Joueur(String nom,Map map,int dimX) throws SlickException {
        ...
        gr=new Grenade();
        try {
        	explosion=new Sound("src/main/ressources/Musique/explosion.ogg");
        	hit=new Sound("src/main/ressources/Musique/hit.ogg");
        } catch(Exception e) {
	...
	public void update(int delta) {
		...
		if (this.isMoving() || this.isJumping()) {
			...
			if(!jumping) {
				base=yPerso;
				if(!map.collision(futurX+LARGEUR_PERSO/2,futurY)) {
					falling=true;
					dy=-16*gravity;
    ...
    /**
    * Determine si le personnage est mort ou en dehors de la carte.
    * @return Le joueur a perdu.
    */
    public boolean exit(){
    	boolean b=false;
    	if (isDead() || yPerso>=map.getHeight()+1-map.getHeightTile()) {
    		b=true;
    	}
 	return b;
    }
